...
export class ScheduleService {
  ...
  getApplicants(interviewType: number, applicantName: string): Observable<BaseResponse<SearchedApplicant[]>>{
    // return of(['Joshua Sho-Silva', 'Stephen Olatunji', 'Nonso Udenwani', 'Bose Adedolapo', 'Olatunji Badejo', 'Funsho Williams', 'Olatunji Babatunde'])
    // .pipe(delay(1000))
  ...
  getStaff(Department: string, StaffName: string): Observable<BaseResponse<StaffName[]>>{
    // return of(['Abayomi Sho-Silva', 'Maria Ayenuro', 'Jennifer Oliseyenuum', 'Joseph Ajibulu', 'Tochukwu Onyeamah', 'Bola Omole'])
    // .pipe(delay(1000))
